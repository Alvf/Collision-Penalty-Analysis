#ifndef BUNNY_STRAND_CLOTH_DROP_H
#define BUNNY_STRAND_CLOTH_DROP_H
...
class BUNNY_STRAND_CLOTH_DROP : public SIMULATION_SCENE {
public:
  ...
  virtual bool buildScene() override
  {
    ...
    _sceneName = "bunny_strand_cloth_drop";
    _writeToFile = true;
    _subStep = 1;
    ...
    _initialAStrand = MATRIX3::Identity();
    // _initialAStrand = MATRIX3::Identity();
    _initialTranslationStrand = VECTOR3(-2.5, 2.0, 0.0);
    // _initialTranslationStrand = VECTOR3(2.0, -1.0, 0.0);
    ...
    _initialA           = M;
    _initialTranslation = half - M * half + VECTOR3(-1.5,-7.3,1.5);
    ...
    _hybridSolver = new HYBRID::VOLUME_BDF_1(*_strandMesh, *_stretchingEnergyStrand, 
                                            *_tetMesh, *_hyperelastic,
                                            *_triangleMesh, *_strechingEnergyShell, *_bendingEnergyShell);

    const int maxNewtonIterations = 5;  // works with warm start
    ...
    centers.push_back(center);
    BOWL* bowlPtr = new BOWL(centers.back(), 0.90,  12.0);
    ...
    _hybridSolver->collisionStiffnessVolume() = collisionMu;
    ...
    _hybridSolver->collisionEpsEdgeEdge() = 0.05;
    _hybridSolver->collisionEpsVertexFace() = 0.1;
  ...
  virtual void stepSimulation(const bool verbose = true) override
  {
    ...
    // const int invisible = _tetMesh->surfaceTriangles().size() - _tetMesh->visibleSurfaceTriangles().size();
    // cout<<"---------- invisible triangles: "<<invisible<<" ---------------"<<endl;
    ...
    if(_frameNumber == 70){
      ...
      _hybridSolver->setShellKinematicConstraint(false);
