#ifndef CURLY_WISPBALL_H
#define CURLY_WISPBALL_H
...
class CURLY_WISPBALL : public SIMULATION_SCENE {
public:
  ...
  virtual bool buildScene() override
  {
    ...
    // curly
    _sceneName = "loose_curls_wisp";
    STRAND_MESH::readSOBJFile("../data/hairball/curly_125.sobj", restVertices, strands);
    //readSOBJFile("../data/hairball/curly_249.sobj", restVertices, strands);
    ...
    //const REAL E = 5e4; // decent for type 4C
    const REAL nu = 0.01;

    const REAL density = 0.065; // 5% of the usual density
    ...
    const REAL tipRadius = 0.025 * 0.5;
    //_tipRadius = 0.1 * 0.5;
    
    cout << " E: " << E << endl;
    cout << " nu: " << nu << endl;
    cout << " density: " << density << endl;
    cout << " radii: " << baseRadius << " " << tipRadius << endl;

    char buffer[256];
    snprintf(buffer, 256, "E=%4.2e", E);
    string Estring(buffer);
    snprintf(buffer, 256, "r0=%.3f_r1=%.3f", baseRadius, tipRadius);
    ...
    if (totalStrands < 1000)
      snprintf(buffer, 256, "s=%d", totalStrands);
    else
      snprintf(buffer, 256, "s=%dK", totalStrands / 1000);
    ...
    _strandMesh = new WISP_MESH(restVertices, strands,
                                E, nu, density, baseRadius, tipRadius);
    const REAL k = 1;
    ...
    // write out the length of a single strand
    cout << " Strand 0 length: " << _strandMesh->strandLengths()[0] << endl;
