#ifndef TET_TWIRL_FAST_SCENE_H
#define TET_TWIRL_FAST_SCENE_H
...
class TET_TWIRL_FAST_SCENE : public SIMULATION_SCENE {
public:
  ...
  TET_TWIRL_FAST_SCENE(const int strands = -1) 
  {
    ...
    cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
    cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
    std::cout << " USING HALF DENSITY " << std:: endl;
    _density = 0.065 * 0.5;
    //std::cout << " USING QUARTER DENSITY " << std:: endl;
    //_density = 0.065 * 0.25;
    //std::cout << " USING 3/4 DENSITY " << std:: endl;
    //_density = 0.065 * 0.75;
    ...
    */
    ...
    // 2000 strands setting
    _baseRadius = 1.0 * 0.5 / sqrt(2);
    _tipRadius = 0.2 * 0.5 / sqrt(2);

    const REAL baseArea = 2000 * M_PI * _baseRadius * _baseRadius;
    const REAL tipArea = 2000 * M_PI * _tipRadius * _tipRadius;
    cout << " base area: " << baseArea << endl;
    cout << " tip area: " << tipArea << endl;
   
    _baseRadius = sqrt(baseArea / _totalStrands / M_PI);
    _tipRadius = sqrt(tipArea / _totalStrands / M_PI);
    cout << " base radius : " << _baseRadius << endl;
    cout << " tip radius: " << _tipRadius << endl;
    
#define USING_SHORT 1
#if !USING_SHORT
    _writeToFile = true;
#else
    _writeToFile = false;
  ...
  virtual bool buildScene() override
  {
    ...
    //cout << " PAUSE FRAME SET TO 50 " << endl;
    //_pauseFrame = 50;
...
cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
cout << " ONLY SIMULATING 20 FRAMES " << endl;
cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << endl;
    //_sceneName = "twirl_wisp_fast_short";
    _sceneName = "twirl";
#else
    _sceneName = "twirl_wisp_fast_800_frames";
#endif
    ...
    REAL collisionEps = 2.0 * _tipRadius;
    const REAL collisionStiffness = 1000;

    if (_totalStrands == 8000)
    {
      //const REAL collision = _strandMesh->collisionEps();
      //_strandMesh->setCollisionEps(2.0 * collision);
      //cout << " BUMPING UP COLLISION RADIUS TO " << 2.0 * collision << endl;
      //_strandMesh->setCollisionEps(0.5 * collision);
      collisionEps *= 0.5;
      cout << " BUMPING DOWN COLLISION RADIUS TO " << collisionEps << endl;
    }
    /*
    if (_totalStrands == 2000)
    {
      collisionEps *= sqrt(2.0);
      cout << " SETTING COLLISION EPS TO " << collisionEps << endl;
    }
    */
    ...
    cout << " Collision eps: " << collisionEps << endl;
    cout << " Collision stiffness: " << collisionStiffness << endl;
    setUnsignedCollisions(collisionStiffness, collisionEps); 
    ...
    // cache out the SOBJ, just for testing
    _strandMeshFilename = _sceneName + string(".sobj");
    STRAND_MESH::writeSOBJFile(_strandMeshFilename.c_str(), _strandMesh->restVertices(), _strandMesh->strandIndices());
  ...
  MATRIX3 shakeHead()
  {
    ...
    //MATRIX3 R = rotationMatrix(axis, 0.2 * cos((3.0 / 5.0) * time * M_PI)); // this amplitude is too violent
