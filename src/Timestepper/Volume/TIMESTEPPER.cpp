...
///////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////
TIMESTEPPER::TIMESTEPPER(TET_MESH& tetMesh, VOLUME::HYPERELASTIC& hyperelastic, ENERGY_12D& vfColl, ENERGY_12D& eeColl) :
  _tetMesh(tetMesh), _hyperelastic(hyperelastic), _damping(NULL), _vfEnergy(vfColl), _eeEnergy(eeColl)
{
  initialize();
}

TIMESTEPPER::TIMESTEPPER(TET_MESH& tetMesh, VOLUME::HYPERELASTIC& hyperelastic, VOLUME::DAMPING& damping, ENERGY_12D& vfColl, ENERGY_12D& eeColl) :
  _tetMesh(tetMesh), _hyperelastic(hyperelastic), _damping(&damping), _vfEnergy(vfColl), _eeEnergy(eeColl)
{
  initialize();
}
...
void TIMESTEPPER::buildConstraintMatrix()
{
  ...
  for (unsigned int x = 0; x < _planeConstraints.size(); x++)
  {
    ...
    if (vertexID == 927)
    {
      ...
      cout << " Sblock 927: " << endl << Sblock << endl;
    }
    */
  }
...
// the "Smoothed aggregation multigrid for cloth simulation" projection matrix
///////////////////////////////////////////////////////////////////////////////////////////////////////
void TIMESTEPPER::buildBlockConstraintMatrix()
{
  TIMER functionTimer(__FUNCTION__);
  BLOCK_DIAGONAL_MATRIX3 I(_DOFs, _DOFs);
  I.setIdentity();
  _blockS = I;
  //std::cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << " : " << std::endl;
  //cout << " Creating constraint matrix of size: " << I.rows() << " " << I.cols() << endl;
  ...
  for (unsigned int x = 0; x < _planeConstraints.size(); x++)
  {
    ...
    const MATRIX3 Sblock = MATRIX3::Identity() - normal * normal.transpose();
    const int vertexID = constraint.vertexID;
    _blockS(vertexID, vertexID) = Sblock;
  ...
  for (unsigned int x = 0; x < _kinematicConstraints.size(); x++)
  {
    ...
    const int vertexID = constraint.vertexID;
    _blockS(vertexID, vertexID).setZero();
  }

  // store the complement
  _blockIminusS = I - _blockS;
}
...
void TIMESTEPPER::computeCollisionDetection()
{
  ...
  if (_vertexFaceSelfCollisionsOn)
  {
    ...
    _tetMesh.computeVertexFaceCollisions(_vfEnergy.eps());
  ...
  if (_edgeEdgeSelfCollisionsOn)
  {
    ...
    _tetMesh.computeEdgeEdgeCollisions(_eeEnergy.eps());
  }
}
...
void TIMESTEPPER::computeCollisionResponse(VECTOR& R, SPARSE_MATRIX& K, SPARSE_MATRIX& collisionC, const bool verbose)
{
  ...
  if (_vertexFaceSelfCollisionsOn)
  {
    ...
    forcesVF = _tetMesh.computeVertexFaceCollisionForces(_vfEnergy);
    hessianVF = _tetMesh.computeVertexFaceCollisionClampedHessian(_vfEnergy);
  ...
  if (_edgeEdgeSelfCollisionsOn)
  {
    forcesEE = _tetMesh.computeEdgeEdgeCollisionForces(_eeEnergy);
    hessianEE = _tetMesh.computeEdgeEdgeCollisionClampedHessian(_eeEnergy);
